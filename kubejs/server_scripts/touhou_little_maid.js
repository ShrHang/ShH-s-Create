ItemEvents.entityInteracted(event => {
    ...
    if (_target.type == 'touhou_little_maid:maid' && _player == _target.getOwner() && _player.isCreative()) {

        if (Platform.isLoaded('shh') && Platform.isLoaded('l2magic') && _item == "shh:maid_tool") {
        ...
        if (!cap) {
            console.log("ShH--Error: Cannot get L2Hostility capability of TouhouLittleMaid.");
            return;
        }
        if (_item == "l2hostility:bottle_of_curse" && !_player.isShiftKeyDown()) {
            cap.setLevel(_target, cap.getLevel() + 100);
        ...
        if (_item == "l2hostility:bottle_of_sanity" && !_player.isShiftKeyDown()) {
            cap.setLevel(_target, 0);
            cap.reinit(_target, cap.getLevel(), true);
            cap.syncToClient(_target);
            if (!_player.isCreative()) {
                event.item.count--
            }
