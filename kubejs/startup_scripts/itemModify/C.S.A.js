    ...
    ["sword", "axe", "pickaxe", "shovel"].forEach(tool => {
        event.modify("create_sa:copper_" + tool, item => {
            addAttModifiers(item, [
                ...
                { attribute: "minecraft:generic.attack_damage", amount: tool === "sword" ? 6.5 : tool === "axe" ? 8 : 0, id: "shh_attack_damage:mainhand", slot: "mainhand", condition: (tool === "sword" || tool === "axe") }
            ]);
        });
    });
    //#endregion
    ...
    [
        {
            ...
            armor: 4,
            modifiers: slot => [
                { attribute: "irons_spellbooks:lightning_spell_power", amount: 0.05, id: `shh_lightning_spell_power:${slot}`, slot: slot, condition: Platform.isLoaded("irons_spellbooks") }
            ]
        },
        {
            material: "zinc",
        ...
        pieceSlot.forEach(ps => {
            event.modify(`create_sa:${mat.material}_${ps.piece}`, item => {
                let mods = [{ attribute: "minecraft:generic.armor", amount: mat.armor, id: `shh_armor:${ps.slot}`, slot: ps.slot }];
